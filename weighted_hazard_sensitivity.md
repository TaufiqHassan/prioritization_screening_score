...
Spearman correlation compares the ranked tract list under each alternative to the ranked tract list under 50/50.
...
The two components use different pollutant subsets. `E` is based on criteria emissions plus toxic emissions for components without available TWE health values. `T` is based only on toxicity-weighted toxic emissions, using the cancer, chronic, and acute TWE endpoints. Criteria pollutants are not included in TWE.
